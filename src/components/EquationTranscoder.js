...
const EquationTranscoder = () => {
    ...
    const handleClick = useCallback(async () => {
        if (!image)
            return;
        if (!geminiApiKey) {
            alert("Gemini API key is missing. Please provide it in the .env.local file.");
            return;
        }
        setText("数式を認識中...");
        ...
        reader.onload = async (event) => {
            ...
            try {
                ...
                if (response.candidates && response.candidates.length > 0 && response.candidates[0].content?.parts?.[0]?.text) {
                    const latexText = removeMarkdown(response.candidates[0].content.parts[0].text);
                    setText(latexText);
                    setRenderedEquation(katex.renderToString(latexText, {
                        displayMode: true,
                        output: "html",
                        trust: true,
                        strict: false,
                ...
                setText(`エラーが発生しました。もう一度お試しください。<br>
          ${errorMessage}`);
            }
            finally {
                setProgress(100);
    ...
    const handleExplainClick = useCallback(async () => {
        ...
        setExplanation([_jsx("p", { children: "\u8AAC\u660E\u3092\u751F\u6210\u4E2D..." }, "loading")]);
        try {
            const response = await ai.models.generateContent({
            ...
            if (response.candidates && response.candidates.length > 0 && response.candidates[0].content?.parts?.[0]?.text) {
                const explanationText = removeMarkdown(response.candidates[0].content.parts[0].text);
                ...
                while ((match = latexRegex.exec(explanationText)) !== null) {
                    ...
                    if (match.index > lastIndex) {
                        parts.push(_jsx("span", { children: explanationText.substring(lastIndex, match.index) }, `text-${lastIndex}`));
                    }
                    // Render LaTeX block
                    ...
                    try {
                        ...
                        parts.push(_jsx("span", { dangerouslySetInnerHTML: { __html: renderedMath } }, `latex-${match.index}`));
                    }
                    catch (error) {
                        const errorMessage = error instanceof Error ? error.message : '不明なエラーが発生しました';
                        console.error("Error rendering LaTeX", errorMessage);
                        parts.push(_jsx("span", { children: match[0] }, `latex-error-${match.index}`)); // Fallback to raw LaTeX if rendering fails
                    }
                    lastIndex = latexRegex.lastIndex;
                }
                // Add remaining plain text
                if (lastIndex < explanationText.length) {
                    parts.push(_jsx("span", { children: explanationText.substring(lastIndex) }, `text-${lastIndex}`));
                }
                setExplanation(parts);
            }
            else {
                setExplanation([
                    _jsx("span", { children: "\u8AAC\u660E\u304C\u898B\u3064\u304B\u308A\u307E\u305B\u3093\u3067\u3057\u305F\u3002" }, "no-explanation"),
                ]);
            }
        }
        catch (error) {
            const errorMessage = error instanceof Error ? error.message : '不明なエラーが発生しました';
            setExplanation([_jsxs("span", { children: ["Error: ", errorMessage] }, "error")]);
        }
    ...
    const handleSVGAction = useCallback(async () => {
        ...
        try {
            ...
            const clonedElement = renderedEquationElement.cloneNode(true);
    ...
    return (_jsxs("div", { className: "equation-transcoder", onPaste: handlePaste, children: [imageURL && (_jsx("div", { children: _jsx("img", { src: imageURL, alt: "Selected Image", style: { maxWidth: "300px" } }) })), _jsxs("div", { className: "file-input-container", onDrop: handleDrop, onDragOver: handleDragOver, children: [_jsx("input", { className: "file-input", type: "file", onChange: handleChange }), _jsx("button", { className: "execute-button", onClick: handleClick, disabled: !image, children: "\u5B9F\u884C" })] }), progress > 0 ? (_jsx("progress", { value: progress, max: 100, className: "progress-bar", style: { opacity: 1 } })) : (_jsx("progress", { className: "progress-bar", style: { opacity: 0 } })), text && (_jsxs("div", { className: "equation-container", children: [_jsx("h3", { children: image ? "認識された数式:" : "入力された数式:" }), _jsx("div", { className: "rendered-equation", dangerouslySetInnerHTML: { __html: renderedEquation || "" } }), _jsxs("div", { className: "equation-text", children: [_jsx("div", { className: "equation-text-label", children: "Tex" }), _jsx("div", { className: "equation-text-main", onClick: () => navigator.clipboard.writeText(text), dangerouslySetInnerHTML: { __html: text } })] }), _jsxs("div", { className: "button-container", children: [_jsx("button", { className: "copy-button", onClick: handleCopy, children: "Tex\u306E\u30B3\u30FC\u30C9\u3092\u30B3\u30D4\u30FC" }), _jsx("button", { className: "explanation-button", onClick: handleExplainClick, children: "\u5F0F\u306E\u8AAC\u660E" }), _jsxs("div", { className: "svg-controls", children: [_jsxs("label", { className: "svg-toggle", children: [_jsx("input", { type: "checkbox", checked: svgCopyMode, onChange: (e) => setSvgCopyMode(e.target.checked) }), _jsx("span", { className: "toggle-label", children: svgCopyMode ? "クリップボードにコピー" : "ファイルとしてダウンロード" })] }), _jsx("button", { className: "download-button", onClick: handleSVGAction, children: svgCopyMode ? "画像をコピー" : "画像をダウンロード" })] })] }), explanation && (_jsxs("div", { className: "explanation-container", children: [_jsx("h3", { children: "\u8AAC\u660E:" }), _jsx("p", { className: "explanation-text", children: explanation })] }))] })), snackbarVisible && (_jsx("div", { className: "snackbar", children: svgCopyMode ? "クリップボードにSVGをコピーしました！" : "クリップボードにTexのコードをコピーしました！" }))] }));
};
